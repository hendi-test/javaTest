...
public class CtsMainAccountUnsuccessRejectTotalDailyReportJob extends AbstractReportJob {

  @Override
  public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {

    log.info(
        "CtsMainAccountUnsuccessRejectTotalDailyReportJob execute method started");

    setJobContext(jobExecutionContext);
    try {
      getReportBean().generateCtsMaUnsRejectTotalDailyReport(get2FromDate(), get2ToDate());
    } catch (RuntimeException ex) {
      log.error("CtsMainAccountSuccessPerChannelDailyReportJob error - " + ex.getMessage());
      throw new TopUpJobExecutionException(ex);
    }
    log.info(
        "CtsMainAccountUnsuccessRejectTotalDailyReportJob execute method finished");
  }
